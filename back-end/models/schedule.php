...
class Schedule
{
    ...
    public function getMovieSchedules($database, $params)
    {
        ...
        $query = "SELECT id_schedule, date, time, COUNT(status) AS seat 
                  ...
                  id_movie = '" . $params['id'] . "' AND status = 0 AND DATE(date) >= "
                  . $curr_date ." GROUP BY id_schedule ;";
    ...
    public function getMovieOnSchedule($database, $params)
    {
        $query = "SELECT nama, date, time FROM " . $this->table .
                 " NATURAL JOIN engima.movie WHERE id_schedule = '" . $params['schedule'] . "';";
        $execute = mysqli_query($database, $query);
        $result = mysqli_fetch_all($execute, MYSQLI_ASSOC);
        if ($result) {
            return $result;
